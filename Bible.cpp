...
int Bible::buildIndex(string infile){
   ifstream filestream;
   int position;
   string line;

   filestream.open(infile.c_str(), ios::in);
   if (!filestream) {
...
std::streampos Bible::getOffset(const Ref& ref) const {
    auto it = bibleIndex.find(ref);
